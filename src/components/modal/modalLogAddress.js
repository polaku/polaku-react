...
export default class modalLogAddress extends Component {
  state = {
    data: [],
    page: 0,
    rowsPerPage: 5,
      ...
      >
        <Fade in={this.props.status}>
          <Grid style={{
            ...
            <Typography style={{ textAlign: 'center', fontWeight: 'bold', marginBottom: 15 }}>Riwayat Perubahan Alamat</Typography>
              ...
              <TableBody>
                {
                  this.state.data.slice(this.state.page * this.state.rowsPerPage, this.state.page * this.state.rowsPerPage + this.state.rowsPerPage).map((element, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell style={{ padding: '10px 15px 10px 15px' }}> {element.address}</TableCell>
